...
export default function PrintIDVerificationScreen() {
  ...
  const { id: documentId } = useLocalSearchParams(); // Get documentId from route params
  ...
  const handlePinSubmit = async () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
    if (pin === '123456') {
      // Pass the document ID and mode to the next screen
      router.replace(`/printable-id-preview?id=${documentId}&mode=print`); // Pass documentId and mode
    } else {
        ...
        if (newAttempts === 0) {
          ...
          Alert.Alert( // Corrected Alert.alert to Alert.alert
            'Incorrect PIN',
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            setTimeout(() => {
              // For biometric login, we might not have a documentId, so navigate without it or with a default mode
              router.replace('/printable-id-preview?mode=print'); // Assuming mode=print is appropriate here too, or adjust as needed
            }, 1000);
          }
        }
      ]
